...
def genGraph(lofd, emotionCol, totaltweets):
    ...
    for i in range(0,len(lofd)):
        thedata.append('[new Date('
                       ...
                       + str((emotionCol["happy"]['h'][i])/float(totaltweets) * 100) + ","
                       + str((emotionCol["sad"]['h'][i])/float(totaltweets) * 100)  + ","
                       + str((emotionCol["confident"]['h'][i])/float(totaltweets) * 100)  + ","
                       + str((emotionCol["worried"]['h'][i])/float(totaltweets) * 100) + ","
                       + str((emotionCol["excited"]['h'][i])/float(totaltweets) * 100) + ","
                       + str((emotionCol["bored"]['h'][i])/float(totaltweets) * 100) + "],\n")
